...
sgetline(char *buf, char **line, int *ptr)
{
	*line = malloc(BUFSIZ);
	...
	if (buf[*ptr] == '\r') {
		/*
		 * check if we reached an end \r\n, which would mean we'd go
		 * out of bounds on the next iteration
		 */
		if (*ptr + 2 >= BUFSIZ)
			return -1;
	...
	/*
	 * don't increment if we reach EOF otherwise we'd be out of bounds,
	 * also end reading
	 */
...
main()
{
	...
	for (;;) {
		/*
		 * add trigger event, this will trigger when one of our
		 * queued events (see above in kevent() call) meets the
		 * filter, and XS	   * will store the event information
		 * here. we add the timespec timeout, zero valued, so kevent
		...
		if (ret > 0) {
			...
			int		ptr = 0;
			/*
			 * send buffer, get a line back, pass the ptr of the
			 * last read character, and pass a count of the read
			 * characters
			 */
